...
class virtualChinrest {
  ...
  trial(display_element, trial) {
    ...
    const resetAndWaitForBallStart = () => {
      const rectX =
        this.container.getBoundingClientRect().width - this.ball_size;
    ...
    const finishBlindSpotPhase = () => {
      ...
      blindspot_config_data["avg_ball_pos"] = accurateRound(
        sum / ballPosLen,
        2
      );
      // calculate distance between avg ball position and square
      const ball_sqr_distance =
        (blindspot_config_data["square_pos"] -
          blindspot_config_data["avg_ball_pos"]) /
        trial_data["px2mm"];
    ...
    const drawBall = () => {
      ...
      this.ball = ball;
      blindspot_config_data["square_pos"] = accurateRound(
        getElementCenter(square).x,
        2
      );
    };
    const animateBall = () => {
    ...
    const recordPosition = () => {
      ...
      blindspot_config_data["ball_pos"].push(
        accurateRound(getElementCenter(this.ball).x, 2)
      );
      //counter and stop
      this.reps_remaining--;
...
virtualChinrest.info = info;

export { virtualChinrest as default };
